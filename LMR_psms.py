...
def get_psm_class(psm_type):
    psm_classes = {'linear': LinearPSM}
    return psm_classes[psm_type]
...
class LinearPSM(BasePSM):
    ...
    def calibrate(self, C, proxy_data,
        ...
        self.slope, self.intercept, r_value, p_value, std_err = linregress(reg_xa,reg_ya)
    ...
    # GH: I don't know how this ran in the first place; must exploit some global namespace
                ...
                pylab.savefig('proxy_%s_%s_LinearModel_calib.png' % (proxy_data.type.replace(" ", "_"),proxy_data.id),bbox_inches='tight')
                pylab.close()
